...
// on the on-device Bonsai-1.7B model because the check is short
...
export async function runGuardrailRewrite(
  ...
  const routeReason = decision.reason || 'Routed guardrail review to on-device Bonsai-1.7B.';
  ...
  if (detectInsufficient(resp.output)) {
    ...
    return {
      ...
      privacy: {
        ...
        modelName: resp.model || 'bonsai-1.7b',
        tier,
        reason: routeReason,
        dataEgressBytes: 0,
        sources: localSources(local),
