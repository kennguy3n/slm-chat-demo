...
describe('DeviceCapabilityPanel', () => {
  ...
  it('renders model name, loaded badge, quant, RAM, and sidecar from the backend', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('device-capability-model')).toHaveTextContent('bonsai-8b');
    });
  ...
  it('clicking "Load model" POSTs to /api/model/load', async () => {
    fetchSpy
      ...
      .mockResolvedValueOnce(jsonResponse({ loaded: true, model: 'bonsai-8b' }))
      .mockResolvedValue(
        jsonResponse({
          loaded: true,
          model: 'bonsai-8b',
  ...
  it('clicking "Unload model" POSTs to /api/model/unload', async () => {
    fetchSpy
      ...
      .mockResolvedValueOnce(jsonResponse({ loaded: false, model: 'bonsai-8b' }))
      .mockResolvedValue(
        jsonResponse({
          loaded: false,
          model: 'bonsai-8b',
